...
Page({
    ...
    async onLoad(param) {
        ...
        this.updataInfo(currentConference);
     ...
     */
    async onShow() {
        // console.log(`ppp`, this.data.currentConference)
        // this.updata();
    },

    updataInfo(conference) {
        const signType = conference.sign_type;
        let tabBarList = this.data.tabBarList;
        if (signType == 0) {
            tabBarList[0].text = '签到';
            tabBarList[1].text = '请假';
        }
        if (signType == 1) {
            tabBarList[0].text = '已签到';
        }
        if (signType == 3) {
            tabBarList[0].text = '已签到';
            tabBarList[1].text = '已请假';
        }
        this.setData({
    ...
    detailTabBar(e) {
        ...
        console.log(`signType:`, signType, `;currentIndex:`, currentIndex);
        if ((signType == 1 || signType == 3) && currentIndex == 0) {
            InterAction.fnShowToast(`已签到`, 'none', 1500);
            console.log(`已签到`);
        }
        if (signType == 3 && currentIndex == 1) {
            InterAction.fnShowToast(`已请假`, 'none', 1500);
            console.log(`已请假`);
        } else {
            console.log(`跳转页面`);
